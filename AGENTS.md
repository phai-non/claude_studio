...
**Codex Studio** — `.Codex/` 폴더의 agents · slash commands · AGENTS.md를 GUI로 만들고 편집하는 Tauri v2 데스크톱 앱. 패키지 식별자는 `Codex-studio`이고 GitHub 슬러그는 `UkiDelly/claude_studio` (앱 업데이트 체크가 이 슬러그에 하드코딩됨 — `apps/desktop/src-tauri/src/commands/app_update.rs`).
...
2. **Rust + Tauri v2** (`apps/desktop/src-tauri/src/`) — 파일 I/O, `Codex` CLI 검사, GitHub Releases polling, MCP stdio introspection, portable-pty 기반 내장 터미널.
...
- `/` → Welcome (프로젝트 열기 / 최근 / Codex CLI 설치 검사 / 앱 업데이트 배너)
- `/settings` → 테마 · 언어 · Marketplace index URL
- `/project/*` → Workspace 셸 + 5개 탭: `agents` · `commands` · `Codex-md` · `marketplace` · `terminal` (`current` 프로젝트 경로는 URL이 아니라 Zustand store에서 읽음)

각 탭은 `src/routes/tabs/`에 얇은 컴포넌트로 있고, 실제 로직은 `src/features/<domain>/`에 모인다.

### feature 모듈 책임

- **`features/agents/`** — `.Codex/agents/*.md` 편집 폼. `lib/schemas/agent.ts` zod 스키마로 frontmatter (`name` kebab-case, `description`, `tools` CSV, `model`)를 실시간 검증하고 자동 수정 제안. 우측 패널 `McpServerGroup`이 MCP 디스커버리(아래) 결과를 펼쳐 툴을 CSV로 추가.
- **`features/commands/`** — `.Codex/commands/*.md`. frontmatter는 `argument-hint`, `allowed-tools`, `description`. body가 슬래시 커맨드 본문.
- **`features/Codex-md/`** — split-view 마크다운 에디터 (좌 입력 / 우 react-markdown + remark-gfm 프리뷰).
...
- **`features/Codex-check/`** — 시작 시 `Codex --version` 실행 후 `https://registry.npmjs.org/@anthropic-ai/Codex/latest`와 비교. 미설치면 설치 페이지로 안내.
...
1. **정적 힌트** (`tool_hints.rs` → `read_tool_hints`)
   - `~/.Codex/settings.json`, 프로젝트 `.Codex/settings.json`, `~/.Codex.json` (top-level + `projects[<현재 path>].mcpServers`), 프로젝트 `.mcp.json`을 모두 읽어 **이름만** 모음.
